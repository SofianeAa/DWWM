...
class MatiereManager
{
	...
	{
 		$db=DbConnect::getDb();
		$q=$db->prepare("INSERT INTO Matiere (libelleMatiere) VALUES (:libelleMatiere)");
	...
	{
 		$db=DbConnect::getDb();
		$q=$db->prepare("UPDATE Matiere SET idMatiere=:idMatiere,libelleMatiere=:libelleMatiere WHERE idMatiere=:idMatiere");
	...
	{
 		$db=DbConnect::getDb();
		$db->exec("DELETE FROM Matiere WHERE idMatiere=" .$obj->getIdMatiere());
	}
	...
	public static function getList()
	{
 		$db=DbConnect::getDb();
		$liste = [];
		$q = $db->query("SELECT * FROM Matiere");
