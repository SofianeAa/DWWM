...
function ajouterLesLettres($val, $tab, $tabpos, $diff)
{
    switch ($diff)
    {
        case 1:
        ...
        case 3:
            ...
            do
            {
                $posRandom = $tabpos[rand(0, count($tabpos) - 1)]; // on choisit une position au hasard
            } while ($tab[$posRandom] == $val); //si cette lettre est déjà dans le motCode, on en choisit une autre
            $tab = ajouterUneLettre($val, $tab, $posRandom); // on place la lettre
            return $tab;
    }
    return -1;
}
...
function testerGagner($nberreur, $tab, $diff)
{
...
{
    $diff = readline("Choisissez un mode de difficultée, 1 pour facile, 2 pour normal et 3 pour difficile : ");

    while ($diff != 2 && $diff != 1 && $diff != 3 && $diff != 10) {
        $diff = readline("Entrez 1, 2 ou 3 pour le mode de difficultée : ");
    }

    echo "\n";
    if ($diff == 1) {
        echo "Vous avez choisis le pendu en mode facile.\n";
    } elseif ($diff == 2) {
        echo "Vous avez choisis le pendu en mode normal.\n";
    } elseif ($diff == 3) {
        echo "Vous avez choisis le pendu en mode difficile.\n";
    } else {
        ($diff == 10);
    }

    echo "Oh, vous vous sentez confiant, bon courage.\n";

    $motATrouver = choisirMot();
    $tabMotAT = str_split($motATrouver);
    $motCode = coderMot($motATrouver, $diff);
    $nberr = 0;
    $gagne = false;
    $mL = [];
    ...
    do {
        ...
        dessinerPendu($nberr);
        $gagne = testerGagner($nberr, $motCode, $diff);
    } while ($gagne == 0);
    if ($gagne == 1) {
